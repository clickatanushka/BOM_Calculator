...
def export_quote():
    ...
    # Use per_board_cost from enriched BOM — do NOT multiply by board_qty here
    bom_cost_per_board = sum(
    ...
    for i, row in enumerate(bom, 1):
        ...
        ws.append([
            ...
            f"€{pb:.4f}"   if pb   else "—",
            f"€{ext:.2f}"  if ext  else "—",
            row.get("mount", ""),
            "DNP" if row.get("dnp") == "Y" else ("No Price" if not unit else "OK"),
        ])

    col_widths = [4, 20, 35, 22, 20, 14, 8, 12, 12, 12, 8, 10]
    for i, w in enumerate(col_widths, 1):
        ws.column_dimensions[get_column_letter(i)].width = w
